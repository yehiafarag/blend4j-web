...
class HistoriesClientImpl extends Client implements HistoriesClient {
  ...
  public ClientResponse downloadDatasetRange(String historyId, String datasetId,
                                             long startRange, long endRange) throws IOException {
    Dataset dataset = showDataset(historyId, datasetId);
    String fileExt = dataset.getDataTypeExt();

    ClientResponse downloadedFile = super.getWebResourceContents(historyId)
            .path(datasetId).path("display").queryParam("to_ext", fileExt).queryParam("range", "bytes=" + startRange + "-" + endRange + "")
