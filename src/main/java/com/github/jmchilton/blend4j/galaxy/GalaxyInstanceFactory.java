...
public class GalaxyInstanceFactory {
  ...
  public  GalaxyInstance get(final String url, final String apiKey) {
    return get(new DefaultWebResourceFactoryImpl(url, apiKey));
  }

  public  GalaxyInstance get(final String url, final String apiKey, final boolean debug) {
    return get(new DefaultWebResourceFactoryImpl(url, apiKey, debug));
  }
  
  public  GalaxyInstance getFromCredentials(final String url, final String email, final String password) {
    return get(new GalaxyAuthWebResourceFactoryImpl(url, email, password));
  }

  public  GalaxyInstance getFromCredentials(final String url, final String email, final String password, final boolean debug) {
    return get(new GalaxyAuthWebResourceFactoryImpl(url, email, password, debug));
  }

  public  GalaxyInstance get(final WebResourceFactory webResourceFactory) {
